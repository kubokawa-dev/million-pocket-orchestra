...
import { NUMBERS3_OFFICIAL_LINKS } from "@/lib/numbers3-official-sources";
import { absoluteUrl } from "@/lib/site";
import { cn } from "@/lib/utils";

type Numbers3DrawPageChromeProps = {
  drawNumber: number;
...
export function Numbers3DrawPageChrome({
  drawNumber,
  row,
  prevDraw,
  nextDraw,
  sameMonthDraws,
}: Numbers3DrawPageChromeProps) {
  const pageUrl = absoluteUrl(`/numbers3/result/${drawNumber}`);
  const shareTitle = `第${drawNumber}回 ナンバーズ3 | 宝くじAI`;
    ...
    <div className="mx-auto w-full max-w-[1600px] space-y-4 px-4 pt-6 sm:px-6">
      ...
      <Card className="border-border/80 shadow-sm ring-1 ring-black/5 dark:ring-white/10">
        ...
        <CardContent className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
          {NUMBERS3_OFFICIAL_LINKS.map((item) => (
            <a
              key={item.href}
              href={item.href}
              target="_blank"
