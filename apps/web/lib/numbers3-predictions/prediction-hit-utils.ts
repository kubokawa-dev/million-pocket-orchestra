...
export function normalizeNumbers3(num: string | null | undefined): string | null {
  if (num == null) return null;
  const d = String(num).replace(/\D/g, "");
  if (d.length < 1 || d.length > 3) return null;
  return d.padStart(3, "0");
}

export function digitsSortedKey(num: string | null | undefined): string | null {
  const n = normalizeNumbers3(num);
  if (!n) return null;
  return [...n].sort().join("");
}
...
export function classifyHit(
  ...
  const p = normalizeNumbers3(prediction);
  const w = normalizeNumbers3(winning);
  if (!p || !w) return "none";
