...
} from "@/lib/numbers3-predictions/ensemble-contributors";
import { getEnsembleWeightJaLabel } from "@/lib/numbers4-predictions/ensemble-weight-labels";
import {
  buildWinningModelHitsForDrawListNumbers3,
  fetchNumbers3DrawFullRow,
  fetchNumbers3DrawResult,
  fetchOfficialWinningDrawsBeforeTargetNumbers3,
  getLatestEnsembleRun,
  loadNumbers3PredictionBundle,
...
import {
  ...
  normalizeNumbers3,
  type HitKind,
} from "@/lib/numbers3-predictions/prediction-hit-utils";
import type {
  AllMiniPlanPayload,
  BudgetPlanSlice,
...
import { Numbers3OfficialDrawDetail } from "./numbers3-official-draw-detail";

const ENSEMBLE_SCORE_HEAD_TITLE =
  "各モデルの予測リストの順位を重み付けして合算し、その後に多様性調整・3桁の合計値ボーナス・ボックスタイプの分布調整などを適用した値です。当選確率や期待値ではありません。";

const ENSEMBLE_NEARBY_HEAD_TITLE =
  "メイン候補に近い別3桁の提案です。統計・LightGBMの桁確率などに基づき生成されます。アンサンブル本体のスコアとは別指標です。";

const ENSEMBLE_CONTRIBUTOR_HEAD_TITLE =
  "下の「モデル別」で読み込んだ各モデル直近ランの候補（最大96件）に、この3桁が含まれるかを表示します。「一致」「ボックス」の文字にマウスを載せるかキーボードでフォーカスすると、モデル名がツールチップで開きます（行は展開しません）。";
...
function buildMethodBoxComboRanking(
  ...
  for (const row of methodRows) {
    for (const pred of row.topPredictions) {
      const normalized = normalizeNumbers3(pred.number);
...
export type Numbers3PredictionsHubProps = {
  /** 指定時はその回の予測のみ読み込み（無ければ公式当選のみ or 404） */
  targetDrawNumber?: number;
  /** `/numbers3/result/[回]` 用の一覧へ戻るリンク */
  showBackToResultList?: boolean;
};

function SourceBadges({ data }: { data: Numbers3PredictionBundle }) {
  const sourceLabel =
...
function PredictionNumberHighlight({
  ...
  const norm = normalizeNumbers3(value);
  const chars =
...
export async function Numbers3PredictionsHub({
  targetDrawNumber,
  showBackToResultList = false,
}: Numbers3PredictionsHubProps = {}) {
  let data: Numbers3PredictionBundle;

  if (targetDrawNumber !== undefined) {
    const got = await loadNumbers3PredictionBundleForDraw(targetDrawNumber);
    if (!got) {
  ...
  const consensus = buildMethodConsensus(data.methodRows, 3, 3);
    ...
    <div className="flex flex-1 flex-col">
      <div className="mx-auto w-full max-w-[1600px] flex-1 space-y-8 px-4 py-8 sm:space-y-10 sm:px-6 sm:py-10">
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-3">
            ...
            <h1 className="text-foreground font-heading text-3xl font-semibold tracking-tight sm:text-4xl">
              第 {data.targetDrawNumber} 回 ナンバーズ3｜予測と当選照合
            </h1>
        ...
        <AnalysisTransparencyCallout
          basis={[
            "サイトに取り込んだナンバーズ3の当せん番号・予測用 JSON（ensemble / method / budget_plan 等）に基づき、一覧・照合・スコア表示をしています。",
        ...
        >
          <CardHeader className="pb-2">
            ...
            <CardDescription>
              ...
              {winningNorm
                ? " 下の予測番号で、当選と同じ数字は出現回数の範囲内だけ左から薄い赤で強調します（例: 当選に同じ数字が1個だけなら、予測側の重複は先頭から順にだけ強調）。"
                : null}
            </CardDescription>
          </CardHeader>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
                  ...
                  <div>
                    ...
                    <CardDescription>
                      ...
                      <code className="font-mono text-xs">
                        predictions/daily/numbers3_{data.targetDrawNumber}.json
                      </code>
                      {" · "}
            ...
            <CardContent className="px-0 pb-6 pt-4 sm:px-0">
              <div className="text-muted-foreground border-border/60 bg-muted/30 mx-4 mb-4 space-y-2 rounded-lg border px-3 py-2.5 text-xs leading-relaxed sm:mx-6">
                <p>
                  ...
                  したうえで、多様性・3桁の合計値・ボックスタイプの分布などを調整した値です。
                  ...
                  <div className="space-y-2 sm:hidden">
                    {topList.length === 0 ? (
                      ...
                      topList.map((row: EnsembleTopPrediction, i: number) => {
                        ...
                        const hit = classifyHit(num, winningRaw);
                        const norm = normalizeNumbers3(num) ?? "";
          ...
          {/* v16: オールミニプラン */}
          <AllMiniPlanCard plan={data.budgetPlan?.all_mini_5} />
