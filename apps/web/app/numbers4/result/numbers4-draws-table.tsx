...
import {
  NUMBERS4_DRAW_COLUMNS,
  type Numbers4DrawRow,
  formatCell,
} from "@/lib/numbers4";

type Numbers4DrawsTableProps = {
  rows: Numbers4DrawRow[];
};

export function Numbers4DrawsTable({ rows }: Numbers4DrawsTableProps) {
  return (
    <div>
      <Table className="min-w-[920px]">
        ...
        <TableBody>
          {rows.length === 0 ? (
            <TableRow>
              <TableCell
                colSpan={NUMBERS4_DRAW_COLUMNS.length + 1}
            ...
            rows.map((row) => (
              ...
              >
                ...
                <TableCell className="px-3 py-2.5 text-right sm:px-4">
                  <Link
                    href={`/numbers4/result/${row.draw_number}`}
                    className={cn(
