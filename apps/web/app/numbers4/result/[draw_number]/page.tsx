...
export async function generateMetadata({
  ...
  return {
    title: `第${n}回`,
    description: `ナンバーズ4 第${n}回の当選番号・払戻情報`,
  };
}

export default async function Numbers4DrawDetailPage({ params }: PageProps) {
  const { draw_number: raw } = await params;
  ...
  const row = data as Numbers4DrawRow;
  ...
  return (
    <div className="flex flex-1 flex-col">
      <div className="mx-auto w-full max-w-3xl flex-1 px-4 py-8 sm:max-w-4xl sm:px-6 sm:py-10">
        <div className="mb-6">
          <Button
            render={<Link href="/numbers4/result" />}
            nativeButton={false}
            variant="ghost"
            size="sm"
