...
def ensure_tables(conn: psycopg2.extensions.connection):
    ...
    # Attempt to ensure auto-increment identity on legacy tables (from SQLite migration)
    try:
        cur.execute("ALTER TABLE numbers4_model_events ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY")
    except Exception:
        pass
    try:
        cur.execute("ALTER TABLE numbers4_predictions_log ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY")
    except Exception:
        pass
    conn.commit()
...
def learn(actual_number: str):
    ...
    conn = get_conn()
    ensure_tables(conn)
    # Ensure identity sequences are synced with current data
