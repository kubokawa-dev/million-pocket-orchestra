...
def save_prediction_to_json(
    ...
    hot_models: Optional[List[Tuple[str, float]]] = None,
    recent_flow: Optional[List[Dict[str, Any]]] = None,
    next_model_predictions: Optional[List[Dict[str, Any]]] = None,
) -> str:
    """
    予測結果をJSONファイルとして保存
    ...
    if hot_models is not None:
        prediction_entry['hot_models'] = [{"model": m, "score": s} for m, s in hot_models]
    if recent_flow is not None:
        prediction_entry['recent_flow'] = recent_flow
    if next_model_predictions is not None:
        prediction_entry['next_model_predictions'] = next_model_predictions
