"""
predictions/daily の JSON を Supabase（PostgREST）の numbers3_daily_prediction_documents に UPSERT。
"""
...
from tools.daily_predictions_json_common import (
    collect_daily_prediction_records_for_lottery,
)
...
def upsert_daily_documents_via_rest(records: list, *, chunk_size: int = 12) -> None:
    ...
    endpoint = (
        f"{base}/rest/v1/numbers3_daily_prediction_documents"
    ...
    print(f"✅ numbers3_daily_prediction_documents に {n} 件 UPSERT しました")


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="predictions/daily JSON（numbers3）→ Supabase PostgREST"
    )
...
def main() -> None:
        ...
        if not base or not key:
            if args.skip_if_unconfigured:
                print(
                    ...
                    "numbers3_daily_prediction_documents UPSERT をスキップ"
                )
                return
            print(
    ...
    print(f"🔌 PostgREST UPSERT（chunk={args.chunk_size}）…")
    try:
