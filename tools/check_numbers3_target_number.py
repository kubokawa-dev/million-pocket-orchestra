#!/usr/bin/env python3
"""
Numbers3 の指定番号が、手法別予測に含まれるかを検証するツール。
"""
...
def normalize_number(value: str) -> str:
    ...
    if len(s) > 3:
        raise ValueError(f"number must be 3 digits or less: {value}")
    return s.zfill(3)


def detect_latest_draw() -> int:
    files = sorted(PREDICTIONS_DIR.glob("numbers3_*.json"))
    if not files:
        raise RuntimeError("No numbers3_*.json found under predictions/daily")
    latest = files[-1].stem
...
def _check_single_number(draw: int, target_number: str, methods: list[str]) -> str:
    ...
    lines: list[str] = []
    lines.append("# Numbers3 ターゲット番号チェック")
    lines.append("")
    ...
    for method in methods:
        path = PREDICTIONS_DIR / "methods" / method / f"numbers3_{draw}.json"
        if not path.exists():
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Numbers3 target number checker")
    parser.add_argument("--draw", type=int, default=None, help="対象回号（未指定なら最新）")
    parser.add_argument("--number", type=str, default="", help="検証する3桁番号（単体指定）")
    parser.add_argument("--numbers", type=str, default="", help="検証する3桁番号（カンマ区切り）")
    parser.add_argument(
        "--methods",
        type=str,
